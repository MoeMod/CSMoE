...
void EV_FireCannonTiger( event_args_t *args )
{
	...
	if (EV_IsLocal(idx))
		EV_CannonTigerFireEffect(ent->attachment[0] + 7.0 * forward, forward, velocity, idx, 0);
	else
		EV_CannonTigerFireEffect(vecSrc + 40.0 * forward, forward, velocity, idx, 0);
}
