...
void EV_FireCannonPlasma( event_args_t *args )
{
	...
	if (EV_IsLocal(idx))
		EV_CannonPlasmaFireEffect(ent->attachment[0] + 7.0 * forward, forward, velocity, idx, 0);
	else
		EV_CannonPlasmaFireEffect(vecSrc + 40.0 * forward, forward, velocity, idx, 0);
}
