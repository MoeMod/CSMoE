...
void CCSBot::BotTouch(CBaseEntity *other)
{
	...
	if (FClassnameIs(other->pev, "func_breakable") || FClassnameIs(other->pev, "func_door_rotating")) {
...
bool CCSBot::IsBusy() const
{
	if (IsAttacking() || IsBuying() || IsDefusingBomb() || GetTask() == PLANT_BOMB || GetTask() == RESCUE_HOSTAGES ||
	    IsSniping() || IsRushingForSupplyBox() || IsDefending()) {
		return true;
	}
...
bool CCSBot::IsHurrying() const
{
	...
	if (ctrl->GetScenario() == CCSBotManager::SCENARIO_ZOMBIEMOD_INFETCION)
		return true;
...
void CCSBot::SetKiller(CBaseEntity* killer)
{
	m_killer = killer;
}
