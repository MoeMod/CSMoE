...
void CCSBotManager::ValidateMapData()
{
	...
	for (i = 1; i < gpGlobals->maxEntities; ++i)
	{
		...
		if (g_pModRunning)
			m_gameScenario = g_pModRunning->OverrideGameScenario(m_gameScenario);
...
bool CCSBotManager::AddBot(const BotProfile *profile, BotProfileTeamType team)
{
	...
	int index = pBot->entindex();
	mp->m_iPlayersOS[index-1] = (OperationSystem)RANDOM_LONG(OS_Windows, OS_Android);
	ClientPutInServer(pBot->edict());
	SET_CLIENT_KEY_VALUE(index, GET_INFO_BUFFER(pBot->edict()), "*bot", "1");
