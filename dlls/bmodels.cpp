...
void CFuncRotating::KeyValue(KeyValueData *pkvd)
{
	if (FStrEq(pkvd->szKeyName, "fanfriction")) {
		...
		Vector tmp;
		UTIL_StringToVector(tmp.data(), pkvd->szValue);
...
void CFuncRotating::RampPitchVol(int fUp)
{
	...
	EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, (char *) STRING(pev->noiseRunning), fvol, m_flAttenuation,
...
void CFuncRotating::SpinUp()
{
	...
	if (abs((int) vecAVel.x) >= abs((int) (pev->movedir.x * pev->speed))
		...
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, (char *) STRING(pev->noiseRunning), m_flVolume, m_flAttenuation,
		               (SND_CHANGE_PITCH | SND_CHANGE_VOL), FANPITCHMAX);
...
void CFuncRotating::RotatingUse(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	if (pev->spawnflags & SF_BRUSH_ACCDCC) {
		...
		{
			SetThink(&CFuncRotating::SpinUp);
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, (char *) STRING(pev->noiseRunning), 0.01, m_flAttenuation, 0,
			               FANPITCHMIN);
	...
	} else if (!(pev->spawnflags & SF_BRUSH_ACCDCC)) // this is a normal start/stop brush.
	{
		if (pev->avelocity != g_vecZero) {
			...
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, (char *) STRING(pev->noiseRunning), m_flVolume, m_flAttenuation, 0,
			               FANPITCHMAX);
			pev->avelocity = pev->movedir * pev->speed;
...
void CPendulum::PendulumUse(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	if (pev->speed) {
		if (pev->spawnflags & SF_PENDULUM_AUTO_RETURN) {
			...
			pev->nextthink = pev->ltime + (delta / m_maxSpeed) * 1s;
