...
#ifdef XASH_DEDICATED
void SV_Update_Tips_f()
{
	char szText[128];
	sprintf(szText, "^2服务器即将关闭更新，请在^3 5~10 ^2秒后重新进入!!");
	...
	char szHudText[128];
	sprintf(szHudText, "服务器即将关闭更新，请在 5~10 秒后重新进入!!");
	...
	textParms.fadeoutTime = 0.5f;
	textParms.holdTime = 10.0f;
	textParms.fxTime = 0.5f;
	textParms.channel = 0;
	UTIL_HudMessageAll(textParms, szHudText);

	ALERT(at_console, "Server will close for updating, please reconnect after 5 to 10 seconds\n");
}
#endif
...
void SV_AddBanWeapon()
{
	...
	for (int i = 1; i < n; ++i)
	{
		int iWeaponID = Q_atoi(CMD_ARGV(i));
		mp->m_setBanWeapon.insert(iWeaponID);
		vecTemp.push_back(iWeaponID);
	}

	MESSAGE_BEGIN(MSG_BROADCAST, gmsgMPToCL);
	WRITE_BYTE(0);
	WRITE_BYTE(11); // [1-wpn,2-knife,3-grenade][0-all, 1-add, 2-remove]
...
void SV_RemoveBanWeapon()
{
	...
	{
		int iWeaponID = Q_atoi(CMD_ARGV(i));
		mp->m_setBanWeapon.erase(iWeaponID);
		vecTemp.push_back(iWeaponID);
	}

	MESSAGE_BEGIN(MSG_BROADCAST, gmsgMPToCL);
	WRITE_BYTE(0);
	WRITE_BYTE(12); // [1-wpn,2-knife,3-grenade][0-all, 1-add, 2-remove]
	WRITE_BYTE(vecTemp.size());
	for (auto& x : vecTemp)
	{
		WRITE_SHORT(x);
...
void SV_AddBanKnife()
{
	...
	for (int i = 1; i < n; ++i)
	{
		int iKnifeID = Q_atoi(CMD_ARGV(i));
		mp->m_setBanKnife.insert(iKnifeID);
		vecTemp.push_back(iKnifeID);
	}

	MESSAGE_BEGIN(MSG_BROADCAST, gmsgMPToCL);
	WRITE_BYTE(0);
	WRITE_BYTE(21); // [1-wpn,2-knife,3-grenade][0-all, 1-add, 2-remove]
	WRITE_BYTE(vecTemp.size());
	for (auto& x : vecTemp)
	{
		WRITE_BYTE(x);
...
void SV_RemoveBanKnife()
{
	...
	{
		int iKnifeID = Q_atoi(CMD_ARGV(i));
		mp->m_setBanKnife.erase(iKnifeID);
		vecTemp.push_back(iKnifeID);
	}

	MESSAGE_BEGIN(MSG_BROADCAST, gmsgMPToCL);
	WRITE_BYTE(0);
	WRITE_BYTE(22); // [1-wpn,2-knife,3-grenade][0-all, 1-add, 2-remove]
	WRITE_BYTE(vecTemp.size());
	for (auto& x : vecTemp)
	{
		WRITE_BYTE(x);
...
void SV_AddBanGrenade()
{
	...
	for (int i = 1; i < n; ++i)
	{
		int iGrenadeID = Q_atoi(CMD_ARGV(i));
		mp->m_setBanGrenade.insert(iGrenadeID);
		vecTemp.push_back(iGrenadeID);
	}

	MESSAGE_BEGIN(MSG_BROADCAST, gmsgMPToCL);
	WRITE_BYTE(0);
	WRITE_BYTE(31); // [1-wpn,2-knife,3-grenade][0-all, 1-add, 2-remove]
	WRITE_BYTE(vecTemp.size());
	for (auto& x : vecTemp)
	{
		WRITE_BYTE(x);
...
void SV_RemoveBanGrenade()
{
	...
	{
		int iGrenadeID = Q_atoi(CMD_ARGV(i));
		mp->m_setBanGrenade.erase(iGrenadeID);
		vecTemp.push_back(iGrenadeID);
	}

	MESSAGE_BEGIN(MSG_BROADCAST, gmsgMPToCL);
	WRITE_BYTE(0);
	WRITE_BYTE(32); // [1-wpn,2-knife,3-grenade][0-all, 1-add, 2-remove]
	WRITE_BYTE(vecTemp.size());
	for (auto& x : vecTemp)
	{
		WRITE_BYTE(x);
...
void CHalfLifeMultiplay::ServerDeactivate()
{
	...
	UTIL_LogPrintfDetail("Career End\n");
}
...
void EndRoundMVPMessage(int iType)
{
	...
	WRITE_SHORT((int)(gpGlobals->time.time_since_epoch().count() - g_pGameRules->m_fRoundCount.time_since_epoch().count()));
	MESSAGE_END();
}
#endif
void EndRoundMessage(const char* sentence, int event)
{
	...
	if (mp != NULL)
	{
		if (teamTriggered)
		{
			UTIL_LogPrintfDetail("Team \"%s\" triggered \"%s\" (CT \"%i\") (T \"%i\")\n", team, message, mp->m_iNumCTWins, mp->m_iNumTerroristWins);
		}
		else
		{
			UTIL_LogPrintfDetail("World triggered \"%s\" (CT \"%i\") (T \"%i\")\n", message, mp->m_iNumCTWins, mp->m_iNumTerroristWins);
		}
	}

	UTIL_LogPrintfDetail("World triggered \"Round_End\"\n");
}

void ReadMultiplayCvars(CHalfLifeMultiplay* mp)
{
	mp->m_iRoundTime = (int)(CVAR_GET_FLOAT("mp_roundtime") * 60) * 1s;
	mp->m_iC4Timer = (int)CVAR_GET_FLOAT("mp_c4timer") * std::chrono::duration_cast<std::chrono::duration<int, std::ratio<1>>>(1s);
	mp->m_iIntroRoundTime = (int)CVAR_GET_FLOAT("mp_freezetime") * 1s;
	mp->m_iLimitTeams = (int)CVAR_GET_FLOAT("mp_limitteams");


	if (mp->m_iRoundTime > 9min)
	{
		CVAR_SET_FLOAT("mp_roundtime", 9);
		mp->m_iRoundTime = 9min;
	}
	else if (mp->m_iRoundTime < 1min)
	{
		CVAR_SET_FLOAT("mp_roundtime", 1);
		mp->m_iRoundTime = 1min;
	}

	if (mp->m_iIntroRoundTime > 1min)
	{
		CVAR_SET_FLOAT("mp_freezetime", 60);
		mp->m_iIntroRoundTime = 1min;
	}
	else if (mp->m_iIntroRoundTime < 0s)
	{
		CVAR_SET_FLOAT("mp_freezetime", 0);
		mp->m_iIntroRoundTime = 0s;
	}

	if (mp->m_iC4Timer > 90s)
	{
		CVAR_SET_FLOAT("mp_c4timer", 90);
		mp->m_iC4Timer = 90s;
	}
	else if (mp->m_iC4Timer < 10s)
	{
...
CHalfLifeMultiplay::CHalfLifeMultiplay()
{
	...
	m_tmNextPeriodicThink = invalid_time_point;
	m_bFirstConnected = 0;
	...
	m_iIntroRoundTime += 2s;
	m_fMaxIdlePeriod = m_iRoundTime * 2;

	duration_t flAutoKickIdle = CVAR_GET_FLOAT("mp_autokick_timeout") * 1s;

	if (flAutoKickIdle > 0.0s)
	{
	...
	else if (IsCareer())
	{
		...
		UTIL_LogPrintfDetail("Career Start\n");
	}
	else
	{
		CVAR_SET_FLOAT("pausable", 0);
	...
	m_fRoundCount = {};
	m_fIntroRoundCount = invalid_time_point;
...
#ifdef XASH_DEDICATED
		ADD_SERVER_COMMAND("update_tips", SV_Update_Tips_f);
#endif
		ADD_SERVER_COMMAND("mp_add_ban_weapon", SV_AddBanWeapon);
		ADD_SERVER_COMMAND("mp_remove_ban_weapon", SV_RemoveBanWeapon);
		ADD_SERVER_COMMAND("mp_add_ban_knife", SV_AddBanKnife);
		ADD_SERVER_COMMAND("mp_remove_ban_knife", SV_RemoveBanKnife);
		ADD_SERVER_COMMAND("mp_add_ban_grenade", SV_AddBanGrenade);
		ADD_SERVER_COMMAND("mp_remove_ban_grenade", SV_RemoveBanGrenade);
	...
	if (IS_DEDICATED_SERVER())
	{
		...
		{
			ALERT(at_console, "Executing server config file\n");
	...
	LoadRandomMaps();
	Q_memset(m_iPlayersOS, OS_Windows, sizeof(m_iPlayersOS));
	m_iVoteOptionCount[0] = m_iVoteOptionCount[1] = 0;
...
bool CHalfLifeMultiplay::NeededPlayersCheck(bool& bNeededPlayers)
{
	...
	if (!m_bFirstConnected && m_iNumSpawnableTerrorist != 0 && m_iNumSpawnableCT != 0)
	{
		...
		UTIL_LogPrintfDetail("World triggered \"Game_Commencing\"\n");
...
#ifdef XASH_DEDICATED
				MESSAGE_BEGIN(MSG_ALL, gmsgShowWin);
				WRITE_BYTE(0);
				WRITE_BYTE(5);
				MESSAGE_END();
#endif
...
void CHalfLifeMultiplay::RestartRound()
{
	...
	m_flIntermissionEndTime = invalid_time_point;
	m_flIntermissionStartTime = invalid_time_point;
	m_fTeamCount = invalid_time_point;
	m_iAccountTerrorist = m_iAccountCT = 0;
	m_iHostagesRescued = 0;
...
void CHalfLifeMultiplay::Think()
{
	...
	if (m_fRoundCount == invalid_time_point)
	{
	...
	else
	{
		if (m_fCareerMatchMenuTime + 10s <= gpGlobals->time || !IsBotSpeaking())
		{
			...
			UTIL_LogPrintfDetail("Career Match %d %d %d %d\n", m_iRoundWinStatus, m_iNumCTWins, m_iNumTerroristWins, TheCareerTasks->AreAllTasksComplete());
	...
	if (g_fGameOver)
	{
		...
		if (m_flIntermissionEndTime < gpGlobals->time && !IsCareer())
		{
			if (!UTIL_HumansInGame()		// if only bots, just change immediately
				...
				|| ((m_flIntermissionStartTime + std::chrono::seconds(MAX_INTERMISSION_TIME)) < gpGlobals->time))
			{
...
{
	if (votemap_enabled.value > 0.0f && m_iVoteMapStatus) {
		return false;
	}
...
void CHalfLifeMultiplay::CheckFreezePeriodExpired()
{
	...
	// Log this information
	UTIL_LogPrintfDetail("World triggered \"Round_Start\"\n");
...
void CHalfLifeMultiplay::CheckRestartRound()
{
	...
	if (iRestartDelay > 0)
	{
		...
		// log the restart
		UTIL_LogPrintfDetail("World triggered \"Restart_Round_(%i_%s)\"\n", iRestartDelay, (iRestartDelay == 1) ? "second" : "seconds");
		UTIL_LogPrintfDetail("Team \"CT\" scored \"%i\" with \"%i\" players\n", m_iNumCTWins, m_iNumCT);
		UTIL_LogPrintfDetail("Team \"TERRORIST\" scored \"%i\" with \"%i\" players\n", m_iNumTerroristWins, m_iNumTerrorist);
...
bool CHalfLifeMultiplay::HasRoundTimeExpired()
{
	...
	if (TimeRemaining() > 0s || m_iRoundWinStatus != WINNER_NONE)
	{
		return false;
	}
...
void CHalfLifeMultiplay::InitHUD(CBasePlayer* pl)
{
	...
	MESSAGE_BEGIN(MSG_ONE, gmsgOperationSystem, NULL, pl->edict());
	...
	{
		WRITE_BYTE(m_iPlayersOS[i]);
	}
	MESSAGE_END();

	if (m_setBanWeapon.size()) {
		MESSAGE_BEGIN(MSG_ONE, gmsgMPToCL, NULL, pl->edict());
		WRITE_BYTE(0);
		WRITE_BYTE(10); // [1-wpn,2-knife,3-grenade][0-all, 1-add, 2-remove]
	...
	if (m_setBanKnife.size()) {
		MESSAGE_BEGIN(MSG_ONE, gmsgMPToCL, NULL, pl->edict());
		WRITE_BYTE(0);
		WRITE_BYTE(20); // [1-wpn,2-knife,3-grenade][0-all, 1-add, 2-remove]
		WRITE_BYTE(m_setBanKnife.size());
		for (auto& x : m_setBanKnife) // 注意别超过250个，否则会发不出msg
		{
			WRITE_BYTE(x);
		}
		MESSAGE_END();
	...
	if (m_setBanGrenade.size()) {
		MESSAGE_BEGIN(MSG_ONE, gmsgMPToCL, NULL, pl->edict());
		WRITE_BYTE(0);
		WRITE_BYTE(30); // [1-wpn,2-knife,3-grenade][0-all, 1-add, 2-remove]
		WRITE_BYTE(m_setBanGrenade.size());
		for (auto& x : m_setBanGrenade) // 注意别超过250个，否则会发不出msg
		{
			WRITE_BYTE(x);
		}
		MESSAGE_END();
...
void CHalfLifeMultiplay::PlayerThink(CBasePlayer* pPlayer)
{
	...
	if (pPlayer->m_pActiveItem)
	{
		if (pPlayer->m_pActiveItem->IsWeapon())
		{
...
BOOL CHalfLifeMultiplay::FPlayerCanRespawn(CBasePlayer* pPlayer)
{
	...
	if (m_iNumTerrorist > 0 && m_iNumCT > 0)
	{
		...
		if (gpGlobals->time > m_fRoundCount + 20.0s)
		{
			if (fadetoblack.value != 0.0f)
			{
				UTIL_ScreenFade(pPlayer, Vector(0, 0, 0), 3s, 3s, 255, (FFADE_OUT | FFADE_STAYOUT));
			}
...
time_point_t CHalfLifeMultiplay::FlPlayerSpawnTime(CBasePlayer* pPlayer)
{
...
void CHalfLifeMultiplay::DeathNotice(CBasePlayer* pVictim, entvars_t* pKiller, entvars_t* pevInflictor)
{
	...
	if (pKiller->flags & FL_CLIENT)
	{
		...
		if (pevInflictor)
		{
			if (pevInflictor == pKiller)
			{
				...
				{
					if (pPlayer->m_pActiveItem->m_iId == WEAPON_HEGRENADE)
					{
						switch (pPlayer->m_iGrenadeID)
						{
						case GRENADE_M24GRENADE:
							killer_weapon_name = "m24grenade";
							break;

						case GRENADE_FGRENADE2:
							killer_weapon_name = "fgrenade2";
							break;
						...
						case GRENADE_MOONCAKE:
							killer_weapon_name = "mooncake";
							break;

						case GRENADE_PUMPKIN:
							killer_weapon_name = "pumpkin";
							break;

						case GRENADE_CARTFRAG:
							killer_weapon_name = "cartfrag";
							break;

						case GRENADE_HOLYBOMB:
							killer_weapon_name = "holybomb";
							break;

						case GRENADE_SFGRENADE:
							killer_weapon_name = "sfgrenade";
							break;
			...
			else
			{
				...
				if (pGrenade && pGrenade->GetWeaponsId() == WEAPON_NONE)
				{
					switch (pGrenade->m_iGrenadeID)
					{
					case GRENADE_M24GRENADE:
						killer_weapon_name = "m24grenade";
						break;
	...
	else
	{
		// killed by the world
		const char* team = GetTeam(pVictim->m_iTeam);
		UTIL_LogPrintfDetail("\"%s<%i><%s><%s>\" committed suicide with \"%s\" (world)\n", STRING(pVictim->pev->netname), GETPLAYERUSERID(pVictim->edict()),
			GETPLAYERAUTHID(pVictim->edict()), team, killer_weapon_name);
	}
...
time_point_t CHalfLifeMultiplay::FlWeaponTryRespawn(CBasePlayerItem* pWeapon)
{
...
{
	if (m_iVoteMapStatus == 5)
	{
		m_iVoteMapStatus = 0;
		g_fGameOver = TRUE;
		CHANGE_LEVEL(m_vecVoteMaps.front().first.c_str(), NULL);
		return;
	}
...
{
#ifdef XASH_DEDICATED
	if (votemap_enabled.value <= 0.0f) {
		if(bAlert) ALERT(at_console, "votemap is disabled\n");
		return false;
	}

	if (m_iVoteMapStatus) {
		if (bAlert) ALERT(at_console, "votemap currently in process\n");
		return false;
	}

	if (!m_vecVoteMaps.size()) {
...
void SV_Start_VoteMaps()
{
	if (!g_pGameRules->CheckCanStartVoteMaps(true))
	{
		if (!g_pGameRules->m_iVoteMapStatus)
			g_pGameRules->m_flVoteNextThink = gpGlobals->time + 1s;
		return;
	}

	ALERT(at_console, "Start voting maps\n");
	g_pGameRules->m_iVoteMapStatus = 1;
	g_pGameRules->m_flVoteNextThink = gpGlobals->time + 3s;
}

void CHalfLifeMultiplay::LoadRandomMaps()
{
	g_flResetTime = gpGlobals->time;
	...
	if (fFirstTime) {
		fFirstTime = false;
		ADD_SERVER_COMMAND("start_votemap", SV_Start_VoteMaps);
	}

	m_iVoteMapStatus = 0;
	m_flVoteNextThink = invalid_time_point;
	m_vecVoteMaps.clear();
	...
	const char* szCurMap = STRING(gpGlobals->mapname);
	// the first map name in the file becomes the default
	while (true)
	{
		Q_memset(szBuffer, 0, sizeof(szBuffer));
		...
		if (IS_MAP_VALID(szMap))
		{
			m_vecVoteMaps.emplace_back(std::make_pair(szMap, 0));
		}
		else
			ALERT(at_console, "Skipping %s from mapcycle, not a valid map\n", szMap);
	}
	...
	std::shuffle(m_vecVoteMaps.begin(), m_vecVoteMaps.end(), std::random_device());
}

void ShowMenu_VoteExtend(CBasePlayer* player)
{
	PlayerMenu* pMenu = g_MenuMgr.menu_create("是该选择了", [&](CBasePlayer* pPlayer, PlayerMenu* pCurMenu, int iItem) {
		if (iItem == MENU_EXIT || iItem == MENU_TIMEOUT) {
			return;
		}

		if (g_pGameRules->m_iVoteMapStatus != 2) {
			return;
		}

		char szText[128];
		if(iItem == 5)
			Q_sprintf(szText, "%s 选择了^1换图^7 (^2%d票^7)", STRING(pPlayer->pev->netname), ++g_pGameRules->m_iVoteOptionCount[0]);
		else
			Q_sprintf(szText, "%s 选择了^1延长^7 (^2%d票^7)", STRING(pPlayer->pev->netname), ++g_pGameRules->m_iVoteOptionCount[1]);
	...
	pMenu->AddItem("投票更换地图");
	if (!Q_strcmp(gamemode.string, "zbs"))
	pMenu->AddItem("重新开始游戏");
	else if(g_pGameRules->m_iMaxRounds != 0)
		pMenu->AddItem("延长地图回合");
	else if(g_pGameRules->m_iMaxRoundsWon != 0)
		pMenu->AddItem("延长地图胜利");
	else
		pMenu->AddItem("延长地图时间");
	pMenu->Display(player, 0, votemap_tally_delay_time.value * 1s);
}

void ShowMenu_VoteMap(CBasePlayer* player)
{
	PlayerMenu* pMenu = g_MenuMgr.menu_create("请选择下一张地图", [&](CBasePlayer* pPlayer, PlayerMenu* pCurMenu, int iItem) {
		if (iItem == MENU_EXIT || iItem == MENU_TIMEOUT) {
			return;
		}

		int length = min(MAX_CUSTOM_VOTE_MAPS, g_pGameRules->m_vecVoteMaps.size());
		if (iItem < 0 || iItem >= length) {
			return;
		}

		if (g_pGameRules->m_iVoteMapStatus != 2 && g_pGameRules->m_iVoteMapStatus != 6) {
			return;
		}
	...
	size_t length = min(MAX_CUSTOM_VOTE_MAPS, g_pGameRules->m_vecVoteMaps.size());
	for (size_t i = 0; i < length; i++)
	{
		pMenu->AddItem(g_pGameRules->m_vecVoteMaps[i].first);
	}
	pMenu->Display(player, 0, votemap_tally_delay_time.value * 1s);
}
...
void CHalfLifeMultiplay::VoteMapThink()
{
	if (m_flVoteNextThink > gpGlobals->time) {
		if (m_iVoteMapStatus == 1 && m_flVoteTipsThink <= gpGlobals->time) {
			m_flVoteTipsThink = gpGlobals->time + 1s;
			hudtextparms_t textParms;
			textParms.x = -1.0f;
			textParms.y = 0.35f;
			textParms.effect = 0;
			...
			textParms.fadeinTime = 0.0f;
			textParms.fadeoutTime = 0.5f;
			textParms.holdTime = 2.0f;
			textParms.fxTime = 0.0f;
			textParms.channel = 0;

			char szText[128];
			int iCountDown = static_cast<int>(m_flVoteNextThink.time_since_epoch() / 1s - gpGlobals->time.time_since_epoch() / 1s);
			sprintf(szText, "%d 秒后开始投票换图！", iCountDown);
			UTIL_HudMessageAll(textParms, szText);
		}
		return;
	}

	if (!m_iVoteMapStatus) {
		if (votemap_enabled.value <= 0.0f || timelimit.value <= 0.0f) {
			return;
		}
		...
		auto flLeftTime = g_flTimeLimit - gpGlobals->time;
		if (flLeftTime < 1.0s || flLeftTime > 129.0s) {
			return;
		}
	...
	switch (m_iVoteMapStatus)
	{
		// Vote start
	case 1: {
		if (votemap_type.value <= 0.0f) {
			ALERT(at_console, "Voting maps start\n");
		}
		else {
			ALERT(at_console, "Voting extend start\n");
		}
		g_pGameRules->m_iVoteMapStatus = 2;
		g_pGameRules->m_flVoteNextThink = gpGlobals->time + votemap_tally_delay_time.value * 1s;
		...
		while ((pPlayer = UTIL_FindEntityByClassname(pPlayer, "player")) != NULL)
		{
			...
			if (votemap_type.value <= 0.0f) {
				ShowMenu_VoteMap(player);
			}
			else {
				m_iVoteOptionCount[0] = m_iVoteOptionCount[1] = 0;
				ShowMenu_VoteExtend(player);
			}
		}
		break;
	}
	...
	case 2: {
		if (votemap_type.value <= 0.0f) {
			std::sort(m_vecVoteMaps.begin(),
				m_vecVoteMaps.begin() + min(m_vecVoteMaps.size(), MAX_CUSTOM_VOTE_MAPS) - 1,
			...
			{
				m_iVoteMapStatus = 4;
				m_flVoteNextThink = gpGlobals->time + 3s;
				sprintf(szText, "投票结束！地图将更换为：^2%s", m_vecVoteMaps.front().first.c_str());
				ALERT(at_console, "Voting Over. Nextmap will be %s\n",
					m_vecVoteMaps.front().first.c_str());
			}
			else
			{
				m_iVoteMapStatus = 3;
				auto delay = (m_bFreezePeriod ? TimeRemaining() + m_iRoundTime : TimeRemaining()) + 2s;
				g_flResetTime = gpGlobals->time + delay;

				sprintf(szText, "投票结束！回合结束后将更换为：^2%s", m_vecVoteMaps.front().first.c_str());
				ALERT(at_console, "Voting Over. Nextmap will be %s after %.2f secs\n",
					m_vecVoteMaps.front().first.c_str(), delay.count());
			}
			MESSAGE_BEGIN(MSG_BROADCAST, gmsgSayText);
			WRITE_BYTE(0);
			WRITE_STRING(szText);
			...
			if (m_iVoteOptionCount[0] > m_iVoteOptionCount[1]) {
				UTIL_SayTextAll("投票结束！开始投票更换地图");
				g_pGameRules->m_iVoteMapStatus = 6;
				g_pGameRules->m_flVoteNextThink = gpGlobals->time + votemap_tally_delay_time.value * 1s;
				...
				if (!Q_strcmp(gamemode.string, "zbs"))
				{
					...
					UTIL_SayTextAll("投票结束！游戏重新开始");
				}
				else if (g_pGameRules->m_iMaxRounds != 0)
				{
					g_pGameRules->m_iMaxRounds += 2;
					CVAR_SET_FLOAT("mp_maxrounds", g_pGameRules->m_iMaxRounds);
					UTIL_SayTextAll("投票结束！延长^2 2 回合");
				}
				else if (g_pGameRules->m_iMaxRoundsWon != 0)
				{
					g_pGameRules->m_iMaxRoundsWon += 2;
					CVAR_SET_FLOAT("mp_winlimit", g_pGameRules->m_iMaxRoundsWon);
					UTIL_SayTextAll("投票结束！延长^2 2 胜利");
				}
				else
				{
					g_flResetTime = gpGlobals->time + 15min;
					UTIL_SayTextAll("投票结束！延长^2 15 分钟");
				}
			}
		}
		break;
	...
	case 3: {
		if (!m_bRoundTerminating) {
			m_flVoteNextThink = gpGlobals->time + 1s;
			break;
		}
		m_iVoteMapStatus = 4;
		m_flVoteNextThink = gpGlobals->time + 3s;
