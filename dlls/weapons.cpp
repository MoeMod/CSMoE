...
*/
std::map<int, ItemInfo> CBasePlayerItem::ItemInfoArray;
...
short g_sModelIndexEventBombExp;
short g_sModelIndexEventBombExp2;
short g_sModelIndexMoonCakeExp;
short g_sModelIndexHolyWater;

short g_sModelIndexBoomerEffect;
short g_sModelIndexBoomerSprite;
short g_sModelIndexBoomerEffectEx;
short g_sModelIndexPosionEffect;
short g_sModelIndexFrostExp;
short g_sModelIndexFrostGibs;
...
void CBasePlayerItem::DefaultTouch(CBaseEntity *pOther)
{
	...
	if (pPlayer->m_bIsZombie && m_iId != WEAPON_KNIFE && Q_strcmp(STRING(pev->classname), "weapon_zombibomb") && Q_strcmp(STRING(pev->classname), "weapon_zombibomb2") && m_iId != WEAPON_ZOMBIELAW)
	{
		return;
	}

	if (!pPlayer->m_bIsZombie && (!Q_strcmp(STRING(pev->classname), "weapon_zombibomb") || !Q_strcmp(STRING(pev->classname), "weapon_zombibomb2")))
	{
		return;
	}
...
void CBasePlayerWeapon::FireRemaining(int &shotsFired, time_point_t &shootTime, BOOL bIsGlock)
{
	...
	if (bIsGlock)
	{
		...
		PLAYBACK_EVENT_FULL(flag, m_pPlayer->edict(), m_usFireGlock18, 0, g_vecZero, g_vecZero, vecDir.x, vecDir.y,
	...
	else
	{
		...
		PLAYBACK_EVENT_FULL(flag, m_pPlayer->edict(), m_usFireFamas, 0, g_vecZero, g_vecZero, vecDir.x, vecDir.y,
...
void CBasePlayerWeapon::ItemPostFrame()
{
	...
	{
		int iClipBeforeAttack = m_iClip;
		FireRemaining(m_iGlock18ShotsFired, m_flGlock18Shoot, TRUE);
		m_pPlayer->m_pModStrategy->OnAttack_Post(this, iClipBeforeAttack, m_iClip, m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType]);
	}
	...
	if (m_fInReload && m_pPlayer->m_flNextAttack <= UTIL_WeaponTimeBase())
	{
		...
		j = m_pPlayer->m_pModStrategy->AdjustDefaultReloadClip(this, j);
...
void CBasePlayerWeapon::SendWeaponAnim(int iAnim, int skiplocal, float framerate)
{
	...
	MESSAGE_BEGIN(MSG_ONE, SVC_WEAPONANIM2, NULL, m_pPlayer->pev);
	WRITE_BYTE(iAnim);	// sequence number
	WRITE_BYTE(pev->body);		// weaponmodel bodygroup.
	WRITE_SHORT(FixedUnsigned16(framerate, 1 << 8));		// framerate
	MESSAGE_END();
}
...
BOOL CBasePlayerWeapon::DefaultDeploy(const char *szViewModel, const char *szWeaponModel, int iAnim, const char *szAnimExt, int skiplocal, duration_t fDelay, duration_t flTimeWeaponIdle)
{
	...
	{
		pev->body = m_pPlayer->m_bIsFemale ? 1 : 0;
	}
...
void CBasePlayerWeapon::Holster(int skiplocal)
{
	...
	m_pPlayer->m_pModStrategy->Holster_Post(this);
}
	...
	for (int i = 0; i < MAX_ITEM_TYPES; ++i)
	{
		...
		while (pItem != NULL)
		{
			...
			if (FClassnameIs(pItem->pev, "weapon_c4"))
			{
				...
				UTIL_LogPrintfDetail("\"%s<%i><%s><TERRORIST>\" triggered \"Got_The_Bomb\"\n",
...
int CWeaponBox::PackAmmo(int iszName, int iCount, int iMaxAmmo)
{
	if (!iszName)
	{
		// error here
	...
	if (iMaxAmmo == -1) {
		iMaxAmmo = MaxAmmoCarry(iszName);
	}

	if (iMaxAmmo != -1 && iCount > 0)
	{
		GiveAmmo(iCount, (char *)STRING(iszName), iMaxAmmo);
		return TRUE;
	}
