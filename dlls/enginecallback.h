...
inline void	SET_SIZE( edict_t *e, const vec3_t rgflMin, const vec3_t rgflMax ) { return (*g_engfuncs.pfnSetSize)(e, rgflMin, rgflMax); }
...
inline float	VEC_TO_YAW( const vec3_t rgflVector ) { return (*g_engfuncs.pfnVecToYaw)(rgflVector); }
inline void	VEC_TO_ANGLES( const vec3_t rgflVectorIn, vec3_t_ref rgflVectorOut ) { return (*g_engfuncs.pfnVecToAngles)(rgflVectorIn, rgflVectorOut); }
inline void	MOVE_TO_ORIGIN( edict_t *ent, const vec3_t pflGoal, float dist, int iMoveType ) { return (*g_engfuncs.pfnMoveToOrigin)(ent, pflGoal, dist, iMoveType); }
...
inline void	MAKE_VECTORS( const vec3_t rgflVector ) { return (*g_engfuncs.pfnMakeVectors)(rgflVector); }
...
inline void	SET_ORIGIN( edict_t *e, const vec3_t rgflOrigin ) { return (*g_engfuncs.pfnSetOrigin)(e, rgflOrigin); }
...
inline void	BUILD_SOUND_MSG( edict_t *entity, int channel, const char *sample, /*int*/float volume, float attenuation, int fFlags, int pitch, int msg_dest, int msg_type, const vec3_t pOrigin, edict_t *ed ) { return (*g_engfuncs.pfnBuildSoundMsg)(entity, channel, sample, volume, attenuation, fFlags, pitch, msg_dest, msg_type, pOrigin, ed); }
inline void	TRACE_LINE( const vec3_t v1, const vec3_t v2, int fNoMonsters, edict_t *pentToSkip, TraceResult *ptr ) { return (*g_engfuncs.pfnTraceLine)(v1, v2, fNoMonsters, pentToSkip, ptr); }
...
inline int	TRACE_MONSTER_HULL( edict_t *pEdict, const vec3_t v1, const vec3_t v2, int fNoMonsters, edict_t *pentToSkip, TraceResult *ptr ) { return (*g_engfuncs.pfnTraceMonsterHull)(pEdict, v1, v2, fNoMonsters, pentToSkip, ptr); }
inline void	TRACE_HULL( const vec3_t v1, const vec3_t v2, int fNoMonsters, int hullNumber, edict_t *pentToSkip, TraceResult *ptr ) { return (*g_engfuncs.pfnTraceHull)(v1, v2, fNoMonsters, hullNumber, pentToSkip, ptr); }
inline void	TRACE_MODEL( const vec3_t v1, const vec3_t v2, int hullNumber, edict_t *pent, TraceResult *ptr ) { return (*g_engfuncs.pfnTraceModel)(v1, v2, hullNumber, pent, ptr); }
inline void	GET_AIM_VECTOR( edict_t* ent, float speed, vec3_t_ref rgflReturn ) { return (*g_engfuncs.pfnGetAimVector)(ent, speed, rgflReturn); }
...
inline void	PARTICLE_EFFECT( const vec3_t org, const vec3_t dir, float color, float count ) { return (*g_engfuncs.pfnParticleEffect)(org, dir, color, count); }
...
inline int	POINT_CONTENTS( const vec3_t rgflVector ) { return (*g_engfuncs.pfnPointContents)(rgflVector); }
...
inline int	ENG_CHECK_PARM( const char *parm, const char **ppnext ) { return (*g_engfuncs.CheckParm)(parm, ppnext); }
inline void MESSAGE_BEGIN(int msg_dest, int msg_type, const vec3_t pOrigin = {}, edict_t *ed = nullptr) { (*g_engfuncs.pfnMessageBegin)(msg_dest, msg_type, pOrigin, ed); }
...
inline edict_t*	FIND_ENTITY_IN_SPHERE( edict_t *pEdictStartSearchAfter, const vec3_t org, float rad ) { return (*g_engfuncs.pfnFindEntityInSphere)(pEdictStartSearchAfter, org, rad); }
...
inline void	EMIT_AMBIENT_SOUND( edict_t *entity, const vec3_t pos, const char *samp, float vol, float attenuation, int fFlags, int pitch ) { return (*g_engfuncs.pfnEmitAmbientSound)(entity, pos, samp, vol, attenuation, fFlags, pitch); }
...
inline void	GET_BONE_POSITION( const edict_t* pEdict, int iBone, vec3_t_ref rgflOrigin, vec3_t_ref rgflAngles ) { return (*g_engfuncs.pfnGetBonePosition)(pEdict, iBone, rgflOrigin, rgflAngles); }
inline uintptr_t FUNCTION_FROM_NAME( const char *pName ) { return (*g_engfuncs.pfnFunctionFromName)(pName); }
inline const char *NAME_FOR_FUNCTION( uintptr_t function ) { return (*g_engfuncs.pfnNameForFunction)(function); }
inline const char *TRACE_TEXTURE( edict_t *pTextureEntity, const vec3_t v1, const vec3_t v2 ) { return (*g_engfuncs.pfnTraceTexture)(pTextureEntity, v1, v2); }
...
inline void	GET_ATTACHMENT( const edict_t *pEdict, int iAttachment, vec3_t_ref rgflOrigin, vec3_t_ref rgflAngles ) { return (*g_engfuncs.pfnGetAttachment)(pEdict, iAttachment, rgflOrigin, rgflAngles); }
...
inline byte*	LOAD_FILE_FOR_ME( const char *filename, int *pLength ) { return (byte*)(*g_engfuncs.pfnLoadFileForMe)(filename, pLength); }
...
inline void	PLAYER_RUN_MOVE( edict_t *fakeclient, const vec3_t viewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, byte msec ) { return (*g_engfuncs.pfnRunPlayerMove)(fakeclient, viewangles, forwardmove, sidemove, upmove, buttons, impulse, msec); }
...
inline void	STATIC_DECAL( const vec3_t origin, int decalIndex, int entityIndex, int modelIndex ) { return (*g_engfuncs.pfnStaticDecal)(origin, decalIndex, entityIndex, modelIndex); }
...
inline void	PLAYBACK_EVENT_FULL( int flags, const edict_t *pInvoker, unsigned short eventindex, float delay, const vec3_t origin, const vec3_t angles, float fparam1, float fparam2, int iparam1, int iparam2, int bparam1, int bparam2 ) { return (*g_engfuncs.pfnPlaybackEvent)(flags, pInvoker, eventindex, delay, origin, angles, fparam1, fparam2, iparam1, iparam2, bparam1, bparam2); }
inline unsigned char *ENGINE_SET_PVS( const vec3_t org ) { return (*g_engfuncs.pfnSetFatPVS)(org); }
inline unsigned char *ENGINE_SET_PAS( const vec3_t org ) { return (*g_engfuncs.pfnSetFatPAS)(org); }
...
inline void	ENGINE_FORCE_UNMODIFIED( FORCE_TYPE type, const vec3_t mins, const vec3_t maxs, const char *filename ) { return (*g_engfuncs.pfnForceUnmodified)(type, mins, maxs, filename); }
...
inline const char*	GET_KEY_VALUE( const char *infobuffer, const char *key ) { return (*g_engfuncs.pfnInfoKeyValue)(infobuffer, key); }
...
inline bool	REMOVE_KEY_VALUE( char *s, const char *key ) { return (*g_engfuncs.pfnInfo_RemoveKey)(s, key); }
