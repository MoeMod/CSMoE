...
#define GENERATE_TEMPLATE_GETTER(FieldName) namespace FieldName {\
    namespace impl { \
        ...
        template<class T> constexpr auto Get_impl(T &&data, priority_tag<4>) -> decltype(data.Calc##FieldName ()) { return data.Calc##FieldName (); } \
        ...
        template<class T> constexpr auto Has(T &&data) -> decltype(Has_impl(data, priority_tag<2>())) { return Has_impl(data, priority_tag<2>()); } \
