...
class CKnifeStrongKnifeX: public LinkWeaponTemplate<CKnifeStrongKnifeX,
	TGeneralData,
...
{
public:
	static constexpr const char *ClassName = "z4b_strongknifex";
	static constexpr const char *V_Model = "models/z4b/v_strongknifex.mdl";
	static constexpr const char *P_Model = "models/z4b/p_strongknifex.mdl";
...
LINK_ENTITY_TO_CLASS(z4b_strongknifex, CKnifeStrongKnifeX)

void CKnifeStrongKnifeX::PrimaryAttack(void)
{
	BOOL fDidHit = FALSE;
...
void CKnifeStrongKnifeX::SecondaryAttack(void)
{
	BOOL fDidHit = FALSE;
	UTIL_MakeVectors(m_pPlayer->pev->v_angle);
	Vector vecSrc = m_pPlayer->GetGunPosition();
	...
	m_pPlayer->SetAnimation(PLAYER_ATTACK1);
#endif
	SendWeaponAnim(ANIM_STABMISS, UseDecrement() != FALSE);
	...
	m_flNextPrimaryAttack = m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 2s;
