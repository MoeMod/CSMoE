...
BOOL CM4A1Dragon::Deploy(void)
{
	...
	if (m_iWeaponState & WPNSTATE_M4A1_SILENCED)
		return DefaultDeploy("models/v_m4a1dragon.mdl", "models/p_m4a1dragon.mdl", M4A1_DRAW, "rifle", UseDecrement() != FALSE);
	else
		return DefaultDeploy("models/v_m4a1dragon.mdl", "models/p_m4a1dragon.mdl", M4A1_UNSIL_DRAW, "rifle", UseDecrement() != FALSE);
}
...
void CM4A1Dragon::PrimaryAttack(void)
{
	if (m_iWeaponState & WPNSTATE_M4A1_SILENCED)
	{
		if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
			M4A1DragonFire(0.035 + (0.4) * m_flAccuracy, 0.0875s, FALSE);
		else if (m_pPlayer->pev->velocity.Length2D() > 140)
			M4A1DragonFire(0.035 + (0.07) * m_flAccuracy, 0.0875s, FALSE);
		else
			M4A1DragonFire((0.025) * m_flAccuracy, 0.0875s, FALSE);
	}
	else
	{
		if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
			...
			M4A1DragonFire((0.02) * m_flAccuracy, 0.0875s, FALSE);
	}
}

void CM4A1Dragon::M4A1DragonFire(float flSpread, duration_t flCycleTime, BOOL fUseAutoAim)
{
	m_bDelayFire = true;
	m_iShotsFired++;
...
void CM4A1Dragon::Reload(void)
{
	...
	if (pWeapon != NULL && !Q_strcmp(STRING(pWeapon->pev->classname), "knife_knifedragon"))
	{
		...
		if (m_iWeaponState & WPNSTATE_M4A1_SILENCED)
			...
			iAnim = M4A1_UNSIL_RELOADD;

		if (DefaultReload(M4A1_MAX_CLIP, iAnim, 2.25s))
		{
	#ifndef CLIENT_DLL
