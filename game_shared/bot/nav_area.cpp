...
void EditNavAreas(NavEditCmdType cmd)
{
	...
	Vector dir, _2, _3;
	UTIL_MakeVectorsPrivate(pLocalPlayer->pev->v_angle, dir, _2, _3);
...
bool GetGroundHeight(const Vector *pos, float *height, Vector *normal, bool usehull)
{
	Vector to;
	to.x = pos->x;
	to.y = pos->y;
	...
	for (offset = 1.0f; offset < maxOffset; offset += inc)
	{
		...
		if(usehull)
			UTIL_TraceHull(from, to, ignore_monsters, human_hull, ignore, &result);
		else
...
CNavArea *CNavAreaGrid::GetNearestNavArea(const Vector *pos, bool anyZ) const
{
	...
	if (GetGroundHeight(pos, &source.z, nullptr, true) == false)
		return nullptr;
...
CNavArea* CNavAreaGrid::GetRandomSpawnableArea(void)
{
	if (!m_grid)
		return NULL;

	int listIndex = RANDOM_LONG(0, m_gridSizeX - 1) + RANDOM_LONG(0, m_gridSizeY - 1) * m_gridSizeX;
	int areaIndex = RANDOM_LONG(0, m_grid[listIndex].size() - 1);

	NavAreaList::const_iterator iter = m_grid[listIndex].begin();

	std::advance(iter, areaIndex);

	if (!m_grid[listIndex].empty())
	{
		CNavArea* area = *iter;

		//if (area->GetID() is in mapsafearea)
			//return NULL;

		if (area->m_extent.hi.x - area->m_extent.lo.x < 40.0f)
			return NULL;

		if (area->m_extent.hi.y - area->m_extent.lo.y < 40.0f)
			return NULL;

		if (area->GetAttributes() & (NAV_CROUCH | NAV_JUMP))
			return NULL;

		Vector vecSrc = area->m_center + Vector(0, 0, HalfHumanHeight);
		TraceResult tr;

		UTIL_TraceHull(vecSrc, vecSrc, dont_ignore_monsters, human_hull, NULL, &tr);
