...
typedef struct enginefuncs_s
{
	...
	void	(*pfnSetSize)( edict_t *e, const vec3_t rgflMin, const vec3_t rgflMax );
	...
	void	(*pfnSetOrigin)( edict_t *e, const vec3_t rgflOrigin );
	void	(*pfnEmitSound)( edict_t *entity, int channel, const char *sample, /*int*/float volume, float attenuation, int fFlags, int pitch );
	void	(*pfnEmitAmbientSound)( edict_t *entity, const vec3_t pos, const char *samp, float vol, float attenuation, int fFlags, int pitch );
	void	(*pfnTraceLine)( const vec3_t v1, const vec3_t v2, int fNoMonsters, edict_t *pentToSkip, TraceResult *ptr );
	void	(*pfnTraceToss)( edict_t* pent, edict_t* pentToIgnore, TraceResult *ptr );
	int	(*pfnTraceMonsterHull)( edict_t *pEdict, const vec3_t v1, const vec3_t v2, int fNoMonsters, edict_t *pentToSkip, TraceResult *ptr );
	void	(*pfnTraceHull)( const vec3_t v1, const vec3_t v2, int fNoMonsters, int hullNumber, edict_t *pentToSkip, TraceResult *ptr );
	void	(*pfnTraceModel)( const vec3_t v1, const vec3_t v2, int hullNumber, edict_t *pent, TraceResult *ptr );
	const char *(*pfnTraceTexture)( edict_t *pTextureEntity, const vec3_t v1, const vec3_t v2 );
	void	(*pfnTraceSphere)( const vec3_t v1, const vec3_t v2, int fNoMonsters, float radius, edict_t *pentToSkip, TraceResult *ptr );
	void	(*pfnGetAimVector)( edict_t* ent, float speed, vec3_t_ref rgflReturn );
	void	(*pfnServerCommand)( const char *str );
	void	(*pfnServerExecute)( void );
	void	(*pfnClientCommand)( edict_t* pEdict, const char *szFmt, ... );
	void	(*pfnParticleEffect)( const vec3_t org, const vec3_t dir, float color, float count );
	void	(*pfnLightStyle)( int style, const char *val );
	int	(*pfnDecalIndex)( const char *name );
	int	(*pfnPointContents)( const vec3_t rgflVector );
	void	(*pfnMessageBegin)( int msg_dest, int msg_type, const vec3_t pOrigin, edict_t *ed );
	...
	void	(*pfnGetBonePosition)( const edict_t* pEdict, int iBone, vec3_t_ref rgflOrigin, vec3_t_ref rgflAngles );
	uintptr_t (*pfnFunctionFromName)( const char *pName );
	const char *(*pfnNameForFunction)( uintptr_t function );
	void	(*pfnClientPrintf)( edict_t* pEdict, PRINT_TYPE ptype, const char *szMsg ); // JOHN: engine callbacks so game DLL can print messages to individual clients
	void	(*pfnServerPrint)( const char *szMsg );
	const char *(*pfnCmd_Args)( void );		// these 3 added 
	const char *(*pfnCmd_Argv)( int argc );		// so game DLL can easily 
	int	(*pfnCmd_Argc)( void );		// access client 'cmd' strings
	void	(*pfnGetAttachment)( const edict_t *pEdict, int iAttachment, vec3_t_ref rgflOrigin, vec3_t_ref rgflAngles );
	...
	void	(*pfnRunPlayerMove)( edict_t *fakeclient, const vec3_t viewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, byte msec );
	...
	void	(*pfnStaticDecal)( const vec3_t origin, int decalIndex, int entityIndex, int modelIndex );
	int	(*pfnPrecacheGeneric)( const char *s );
	int	(*pfnGetPlayerUserId)( edict_t *e ); // returns the server assigned userid for this player.  useful for logging frags, etc.  returns -1 if the edict couldn't be found in the list of clients
	void	(*pfnBuildSoundMsg)( edict_t *entity, int channel, const char *sample, /*int*/float volume, float attenuation, int fFlags, int pitch, int msg_dest, int msg_type, const vec3_t pOrigin, edict_t *ed );
	...
	void	(*pfnPlaybackEvent)( int flags, const edict_t *pInvoker, unsigned short eventindex, float delay, const vec3_t origin, const vec3_t angles, float fparam1, float fparam2, int iparam1, int iparam2, int bparam1, int bparam2 );
	
	unsigned char* (*pfnSetFatPVS)(const vec3_t org);
	unsigned char* (*pfnSetFatPAS)(const vec3_t org);
	...
	void	(*pfnForceUnmodified)( FORCE_TYPE type, const vec3_t mins, const vec3_t maxs, const char *filename );
...
typedef struct saverestore_s
{
	...
	vec3_c		vecLandmarkOffset;		// for landmark transitions
	float		time;
...
typedef struct 
{
	...
	void	(*pfnCreateBaseline)( int player, int eindex, struct entity_state_s *baseline, struct edict_s *entity, int playermodelindex, const vec3_t player_mins, const vec3_t player_maxs );
	...
	void	(*pfnClientOperationSystem)(edict_t* pEntity, const char* osname);
} DLL_FUNCTIONS;
