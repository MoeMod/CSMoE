...
=================
*/
edict_t *GAME_EXPORT pfnFindEntityInSphere( edict_t *pStartEdict, const vec3_t org, float flRadius )
{
	edict_t	*ent;
	float	distSquared;
	float	eorg;
...
=================
*/
void GAME_EXPORT pfnEmitAmbientSound( edict_t *ent, const vec3_t pos, const char *sample, float vol, float attn, int flags, int pitch )
{
...
=============
*/
void GAME_EXPORT pfnTraceSphere( const vec3_t v1, const vec3_t v2, int fNoMonsters, float radius, edict_t *pentToSkip, TraceResult *ptr )
{
...
=================
*/
void GAME_EXPORT pfnParticleEffect( const vec3_t org, const vec3_t dir, float color, float count )
{
	int	i, v;
...
void *GAME_EXPORT pfnPvAllocEntPrivateData( edict_t *pEdict, long cb )
{
	...
	if( cb > 0 )
	{
		...
		pEdict->pvPrivateData = Mem_AlignedAlloc( svgame.mempool, (cb + 15) & ~15, 16 );
	}
...
void SV_AllocStringPool( void )
{
...
#ifdef USE_MMAP
	{
		...
		size_t arrlen = ( str64.maxstringarray * 2 ) & ~( pagesize - 1 );
		char *base = (char *)svgame.dllFuncs.pfnGameInit;
		char *start = (char *)svgame.hInstance - arrlen;

		while( start - base > INT_MIN )
		{
			char *mapptr = (char *)mmap((void*)((unsigned long)start & ~(pagesize - 1)), arrlen, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, 0, 0 );
		...
		if( !ptr )
		{
			...
			{
				char *mapptr = (char *)mmap((void*)((unsigned long)start & ~(pagesize - 1)), arrlen, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, 0, 0 );
				if( mapptr && mapptr != (char*)-1  && mapptr - base > INT_MIN && mapptr - base < INT_MAX )
				{
					ptr = (char *)mapptr;
					break;
				}
		...
		else
		{
			...
			ptr = str64.staticstringarray = (char *)Mem_Alloc(host.mempool, str64.maxstringarray * 2);
		}
	}
#else
	ptr = str64.staticstringarray = (char *)Mem_Alloc(host.mempool, str64.maxstringarray * 2);
...
=============
*/
static void GAME_EXPORT pfnGetBonePosition( const edict_t* pEdict, int iBone, vec3_t_ref rgflOrigin, vec3_t_ref rgflAngles )
{
	if( !SV_IsValidEdict( pEdict ))
	{
...
=============
*/
uintptr_t GAME_EXPORT pfnFunctionFromName( const char *pName )
{
	return (uintptr_t)Com_FunctionFromName_SR( svgame.hInstance, pName );
}
...
=============
*/
const char *GAME_EXPORT pfnNameForFunction( uintptr_t function )
{
...
=============
*/
static void GAME_EXPORT pfnGetAttachment( const edict_t *pEdict, int iAttachment, vec3_t_ref rgflOrigin, vec3_t_ref rgflAngles )
{
	if( !SV_IsValidEdict( pEdict ))
	{
...
=============
*/
void GAME_EXPORT SV_PlaybackEventFull( int flags, const edict_t *pInvoker, word eventindex, float delay, const vec3_t origin, const vec3_t angles, float fparam1, float fparam2, int iparam1, int iparam2, int bparam1, int bparam2 )
{
	sv_client_t	*cl;
	event_state_t	*es;
	event_args_t	args;
...
=============
*/
void GAME_EXPORT pfnForceUnmodified( FORCE_TYPE type, const vec3_t mins, const vec3_t maxs, const char *filename )
{
...
qboolean SV_LoadProgs( const char *name )
{
	...
	svgame.globals->maxEntities = GI->max_edicts;
	svgame.globals->maxClients = sv_maxclients->integer;
	svgame.edicts = (edict_t*)Mem_AlignedAlloc( svgame.mempool, sizeof( edict_t ) * svgame.globals->maxEntities, alignof(edict_t) );
