...
qboolean SV_CheckWater( edict_t *ent )
{
	...
	if( cont <= CONTENTS_WATER && cont > CONTENTS_TRANSLUCENT )
	{
		...
		if( truecont <= CONTENTS_CURRENT_0 && truecont >= CONTENTS_CURRENT_DOWN )
		{
			...
			auto dir = current_table[CONTENTS_CURRENT_0 - truecont];
...
==================
*/
int SV_ClipVelocity( vec3_t in, vec3_t normal, vec3_t_ref out, float overbounce )
{
	float	backoff;
	int	i, blocked;
...
static edict_t *SV_PushMove( edict_t *pusher, float movetime )
{
    ...
    lmove = pusher->v.velocity * movetime;
    mins = pusher->v.absmin + lmove;
    maxs = pusher->v.absmax + lmove;
...
static edict_t *SV_PushRotate( edict_t *pusher, float movetime )
{
    ...
    amove = pusher->v.avelocity * movetime;
...
{
#ifdef XASH_MEMDBG
	return _Mem_ZeroAlloc( svgame.mempool, cb, filename, fileline );
#else
	return Mem_ZeroAlloc( svgame.mempool, cb );
#endif
}
