...
#if defined __ANDROID__ && (defined(XASH_WES) || defined(XASH_GL4ES))
...
static qboolean R_FollowEntity( cl_entity_t *ent )
{
    ...
    if( ent->curstate.movetype != MOVETYPE_FOLLOW && ent->curstate.movetype != MOVETYPE_FOLLOWMOVE)
        return false;
...
static qboolean R_OpaqueEntity( cl_entity_t *ent )
{
    ...
    if( ent->curstate.rendermode == kRenderTransAlpha )
        return false; // alpha-test breaks early z test， ignore
...
===============
*/
static float R_CalcDistance(cl_entity_t *ent1)
{
    vec3_t		vecLen, org;
    if( ent1->model && ent1->model->type == mod_brush )
    {
    ...
    return VectorLength( vecLen );
}

static bool R_SolidEntityCompare( cl_entity_t *ent1, cl_entity_t *ent2 )
{
    return std::make_pair(ent1->model->type, R_CalcDistance(ent1)) < std::make_pair(ent2->model->type, R_CalcDistance(ent2));
}
...
===============
*/
static bool R_TransEntityCompare( cl_entity_t *ent1, cl_entity_t *ent2 )
{
    // now sort by rendermode
    int render1 = R_RankForRenderMode(ent1), render2 = R_RankForRenderMode(ent2);
    return std::make_pair(render1, R_CalcDistance(ent2)) < std::make_pair(render2, R_CalcDistance(ent1));
}
...
===============
*/
qboolean GAME_EXPORT R_WorldToScreen( const vec3_t point, vec3_t_ref screen )
{
...
===============
*/
void GAME_EXPORT R_ScreenToWorld( const vec3_t screen, vec3_t_ref point )
{
...
void R_ClearScene( void )
{
    ...
    tr.num_child_entities = tr.num_delay_entities = 0;
}
...
qboolean R_AddEntity( struct cl_entity_s *clent, int entityType )
{
    ...
    {
        if (clent->curstate.eflags & EFLAG_AFTER_VIEWMODEL)
        {
            if (tr.num_delay_entities >= MAX_VISIBLE_PACKET)
                return false;

            tr.delay_entities[tr.num_delay_entities++] = clent;
            return true;
        }
...
void R_SetupFrustum( void )
{
    ...
    RI.frustum[0].normal = RotatePointAroundVector( RI.cull_vup, RI.cull_vforward, -( 90 - RI.refdef.fov_x / 2 ));
    // rotate RI.vforward left by FOV_X/2 degrees
    RI.frustum[1].normal = RotatePointAroundVector( RI.cull_vup, RI.cull_vforward, 90 - RI.refdef.fov_x / 2 );
    // rotate RI.vforward up by FOV_X/2 degrees
    RI.frustum[2].normal = RotatePointAroundVector( RI.cull_vright, RI.cull_vforward, 90 - RI.refdef.fov_y / 2 );
    // rotate RI.vforward down by FOV_X/2 degrees
    RI.frustum[3].normal = RotatePointAroundVector( RI.cull_vright, RI.cull_vforward, -( 90 - RI.refdef.fov_y / 2 ));
...
=============
*/
static void R_SetupProjectionMatrix( const ref_params_t *fd, matrix4x4_ref m )
{
...
=============
*/
static void R_SetupModelviewMatrix( const ref_params_t *fd, matrix4x4_ref m )
{
#if 0
...
static void R_SetupFrame( void )
{
    ...
    std::stable_sort( tr.solid_entities, tr.solid_entities + tr.num_solid_entities, R_SolidEntityCompare );

    if( !gl_nosort->integer )
    {
        // sort translucents entities by rendermode and distance
        std::stable_sort( tr.trans_entities, tr.trans_entities + tr.num_trans_entities, R_TransEntityCompare );
    }
...
static void R_CheckFog( void )
{
    ...
    if( RI.refdef.waterlevel < 2 || !RI.drawWorld || !r_viewleaf || !gl_fog->integer )
        return;
...
#if 0 && defined __ANDROID__ && (defined(XASH_WES) || defined(XASH_GL4ES))
    // skip useless lock
...
=================
*/
static void R_EnvShot( const vec3_t vieworg, const char *name, int skyshot, int shotsize )
{
...
{
    auto pic = R_GetTexture( texnum )->original;
...
{
#ifdef XASH_MEMDBG
    return _Mem_ZeroAlloc( cls.mempool, cb, filename, fileline );
#else
    return Mem_ZeroAlloc( cls.mempool, cb );
#endif
}
