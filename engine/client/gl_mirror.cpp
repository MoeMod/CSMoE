...
================
*/
void R_PlaneForMirror( msurface_t *surf, mplane_t *out, matrix4x4_ref m )
{
	cl_entity_t	*ent;
...
int R_AllocateMirrorTexture( void )
{
	...
	{
		// not initialized ?
		image_ref r_screen = Image_NewTemp();
...
void R_RecursiveMirrorNode( mnode_t *node, uint clipflags )
{
	...
	if( clipflags )
	{
		for( i = 0, clipplane = RI.frustum; i < 6; i++, clipplane++ )
		{
			...
			vec3_t a{ node->minmaxs[0], node->minmaxs[1], node->minmaxs[2]};
			vec3_t b{ node->minmaxs[3], node->minmaxs[4], node->minmaxs[5]};

			clipped = BoxOnPlaneSide( a, b, clipplane );
			if( clipped == 2 ) return;
