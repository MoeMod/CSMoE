...
{
	const char	*skybox_ext[] = { "astc", "dds", "tga", "bmp" };
...
void DrawSkyPolygon( int nump, float *vecs )
{
	...
	{
		j = vec_to_st[axis][2];
		dv = (j > 0) ? vp[j-1] : -vp[-j-1];

		if( dv == 0 ) continue;

		j = vec_to_st[axis][0];
		s = (j < 0) ? -vp[-j-1] / dv : vp[j-1] / dv;

		j = vec_to_st[axis][1];
		t = (j < 0) ? -vp[-j-1] / dv : vp[j-1] / dv;
...
void ClipSkyPolygon( int nump, float *vecs, int stage )
{
	...
	vec3_c		newv[2][MAX_CLIP_VERTS + 1];
	int		newc[2];
	int		i, j;
...
loc1:
	...
	// continue
	ClipSkyPolygon( newc[0], (float *)newv[0], stage + 1 );
	ClipSkyPolygon( newc[1], (float *)newv[1], stage + 1 );
}
...
void R_AddSkyBoxSurface( msurface_t *fa )
{
	...
	for( p = fa->polys; p; p = p->next )
	{
		...
		ClipSkyPolygon( p->numverts, (float *)verts, 0 );
	}
}
...
void R_DrawSkyBox( void )
{
	...
	if( clgame.movevars.skyangle && (clgame.movevars.skydir_x || clgame.movevars.skydir_y || clgame.movevars.skydir_z) )
	{
		matrix4x4	m;
...
==============
*/
void R_InitSky( const mip_t *mt, const byte *buf, texture_t *tx )
{
	image_ref	r_sky;
	uint	*trans, *rgba;
	uint	transpix;
	...
	image_ref r_temp = Image_NewTemp();
	*r_temp = *r_sky;
...
void EmitWaterPolys( glpoly_t *polys, qboolean noCull )
{
	...
	for( p = polys; p; p = p->next )
	{
		...
		for( i = 0; i < p->numverts; i++ )
		{
			if( waveHeight )
			{
				nv = r_turbsin[(int)(cl.time * 160.0f + p->verts[i].xyz.y + p->verts[i].xyz.x) & 255] + 8.0f;
				nv = (r_turbsin[(int)(p->verts[i].xyz.x * 5.0f + cl.time * 171.0f - p->verts[i].xyz.y) & 255] + 8.0f ) * 0.8f + nv;
				nv = nv * waveHeight + p->verts[i].xyz.z;
			}
			else nv = p->verts[i].xyz.z;
			...
			pglVertex3f( p->verts[i].xyz.x, p->verts[i].xyz.y, nv );
		}
		pglEnd();
	}
...
void EmitSkyPolys( msurface_t *fa )
{
	...
	for( p = fa->polys; p; p = p->next )
	{
		...
		for( i = 0; i < p->numverts; i++ )
		{
			VectorSubtract( p->verts[i].xyz, RI.vieworg, dir );
