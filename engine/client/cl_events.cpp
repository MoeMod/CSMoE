...
void CL_RegisterEvent( int lastnum, const char *szEvName, pfnEventHook func )
{
	...
	if( !clgame.events[lastnum] )
		clgame.events[lastnum] = (cl_user_event_t*)Mem_AlignedAlloc( cls.mempool, sizeof( cl_user_event_t ), alignof( cl_user_event_t ));
...
=============
*/
void GAME_EXPORT CL_PlaybackEvent( int flags, const edict_t *pInvoker, word eventindex, float delay, const vec3_t origin,
