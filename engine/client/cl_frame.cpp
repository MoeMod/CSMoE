...
qboolean CL_AddVisibleEntity( cl_entity_t *ent, int entityType )
{
	...
	if( CL_IsInMenu( ) && ( ( !ui_renderworld->integer && !cl.background ) || ent->player ))
	{
		...
		if (RP_LOCALCLIENT(ent) && !cl.thirdperson && cl.refdef.viewentity == (cl.playernum + 1))
		{
	...
	{
#ifdef XASH_RAGDOLL
		if (entityType == ET_NORMAL && ent->model)
		{
			if (ent->model->type == mod_brush && ent->curstate.solid == SOLID_BSP)
			{
				physics::gPhysicsManager.CreateBrushModel(ent);
			}
		}
#endif
...
==================
*/
void GAME_EXPORT CL_WeaponAnim2(int iAnim, int body, float framerate)
{
	cl_entity_t* view = &clgame.viewent;
	cl.weaponstarttime = 0;
	cl.weaponseq = iAnim;
