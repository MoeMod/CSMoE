...
double GAME_EXPORT Sys_DoubleTime( void )
{
    ...
    static clock::time_point g_ClockStart;

    if( !g_ClockStart.time_since_epoch().count() )
    {
        g_ClockStart = clock::now();
    }

    auto time = clock::now() - g_ClockStart;
...
#if TARGET_OS_OSX
#ifndef XASH_DEDICATED
	extern void MacOS_OpenURL(const char *url);
	MacOS_OpenURL(path);
...
#else
		dll->link = LoadLibraryW(buffer);
#endif
#else
		dll->link = dlopen(dll->name, RTLD_NOW); // environment pathes
#endif
	}
