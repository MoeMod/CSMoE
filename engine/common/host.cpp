...
void Host_GetHostInfo(float* fps, int* nActive, int* nMaxPlayers, char* pszMap)
{
	...
	*nMaxPlayers = sv_maxclients->integer;
}
...
void UpdateStatus(int force)
{
	...
	Host_GetHostInfo(&fps, &n, &nMax, &szMap[0]);
	...
	Console_SetStatusLine(szStatus);
	Console_UpdateStatus();
}

void ProcessConsoleInput()
{
	char* inputLine = Console_GetLine(&console);
	if (inputLine)
	{
		char szBuf[256];
...
#if defined _WIN32 && defined XASH_DEDICATED
	Host_ComputeFPS(host.frametime);
#endif
...
# endif
	...
	if( !Sys_GetParmFromCmdLine( "-rodir", host.rodir ) )
	{
		...
		{
			Q_strcpy( host.rodir, "/android_asset/");
		}
#endif
	}
...
#elif _WIN32
	wchar_t wrootdir[256];
	MultiByteToWideChar(CP_UTF8, 0, host.rootdir, -1, wrootdir, 256);
	if ( !host.rootdir[0] || SetCurrentDirectoryW( wrootdir ) != 0)
		MsgDev( D_INFO, "%s is working directory now\n", host.rootdir );
	else
		Sys_Error( "Changing working directory to %s failed.\n", host.rootdir );
#else
	...
	Cmd_AddCommand( "lowmem", Host_LowMem_f, "release some memory" );
...
int EXPORT Host_Main( int argc, const char **argv, const char *progname, int bChangeGame, pfnChangeGame func )
{
	...
	host_sleeptime = Cvar_Get( "sleeptime", DEFAULT_SLEEPTIME, CVAR_ARCHIVE, "higher value means lower accuracy" );
